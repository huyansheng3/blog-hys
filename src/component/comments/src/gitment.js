...
export default class Gitment {
  ...
  constructor(options = {}) {
    ...
    if (query.code) {
      ...
      window.location.history.replaceState({}, '', replacedUrl)
  ...
  loadComments(page = this.state.currentPage) {
    return this.getIssue()
      .then(issue =>
        http.get(issue.comments_url, { page, per_page: this.perPage }, '')
      )
      .then(comments => {
  ...
  like() {
    ...
    return http
      .post(
        `/repos/${owner}/${repo}/issues/${this.state.meta.number}/reactions`,
        {
          content: 'heart',
        }
      )
  ...
  unlike() {
    ...
    const index = reactions.findIndex(
      reaction => reaction.user.login === user.login
    )
    return http.delete(`/reactions/${reactions[index].id}`).then(() => {
